...
from ..caching.track_cache import cache_spotify_track, get_cached_spotify_track, get_or_update_playlist_tracks, cache_spotify_track_with_playlist_update, cache_reccobeats_features_with_playlist_update
...
class PlaylistCard(BoxLayout):
    ...
    def __init__(self, playlist_data: Any, **kwargs: Any) -> None:
        ...
        self.movement_threshold = dp(12)  # 12dp movement threshold to distinguish scroll from click
        self.min_click_duration = 0.05  # 50ms minimum duration for real clicks (trackpad scrolling is faster)
    ...
    def _build_card_ui(self) -> None:
        ...
        info_container = BoxLayout(orientation='vertical', size_hint_y=None, height=dp(65))
    ...
    def _get_playlist_image_url(self) -> str:
        try:
            if self.playlist_data.get('images') and len(self.playlist_data['images']) > 0:
    ...
    def is_visible_in_viewport(self) -> bool:
        ...
        try:
            ...
            return (card_top > viewport_bottom - margin and 
                   card_bottom < viewport_top + margin)
    ...
    def on_card_touch_down(self, _instance, touch):
        try:
            ...
            logger.debug(f"[TOUCH DOWN] pos={touch.pos}, button={getattr(touch, 'button', None)}, profile={getattr(touch, 'profile', None)}")
            ...
            self.long_press_event = Clock.schedule_once(
                lambda _dt: self.show_detailed_playlist_window() if self.is_touch_down else None,
                0.8,
            )
            return True
        except Exception as exc:
    ...
    def on_card_touch_up(self, _instance, touch):
        try:
            ...
            logger.debug(f"[TOUCH UP] pos={touch.pos}, duration={touch_duration:.3f}s, movement={movement_distance:.1f}px, threshold={self.movement_threshold:.1f}px")
            
            # If movement exceeds threshold, treat as scroll/pan gesture, not a click
            if movement_distance > self.movement_threshold:
                logger.debug(f"[TOUCH] Ignored as scroll gesture - movement {movement_distance:.1f}px > threshold {self.movement_threshold:.1f}px")
                self.touch_start_pos = None
                return True
            ...
            if touch_duration < self.min_click_duration and movement_distance == 0:
                logger.debug(f"[TOUCH] Ignored as trackpad scroll - duration {touch_duration:.3f}s < min {self.min_click_duration:.3f}s with zero movement")
                self.touch_start_pos = None
                return True
            ...
            if touch_duration < 0.8:
                logger.debug(f"[TOUCH] Processing as click - duration {touch_duration:.3f}s < 0.8s")
                time_since_last_click = current_time - self.last_click_time
                if time_since_last_click < self.double_click_threshold:
                    logger.debug(f"[TOUCH] Double click detected - time since last {time_since_last_click:.3f}s < threshold {self.double_click_threshold:.3f}s")
    ...
    def create_technical_details_widgets(self, owner_id, playlist_id, snapshot_id):
        ...
        if owner_id != 'Unknown' or playlist_id or snapshot_id != 'Unknown':
            ...
            if snapshot_id != 'Unknown':
                display_snapshot = snapshot_id[:20] + ("..." if len(snapshot_id) > 20 else "")
                tech_widgets.append(
                    self.create_info_label(
    ...
    def get_window_bounds(self):
        ...
        self.stats_widget = self.create_info_label(stats_text, font_size=dp(16), height=dp(12))
        info_layout.add_widget(self.stats_widget)

    def _add_show_tracks_button(self, info_layout: BoxLayout, playlist_info: Dict[str, Any]) -> None:
        try:
            for widget in self.master_content_container.children:
                if getattr(widget, 'text', '') == playlist_info['playlist_name']:
                    ...
                    self.master_content_container.add_widget(title_container, index=len(self.master_content_container.children))
                    break
        except Exception as exc:
        ...
        try:
            ...
            has_cached_reccobeats = bool(
                ...
                and any(
                    ...
                    for key in ['mood_prediction', 'audio_features', 'musical_characteristics', 'mood_analysis']
                )
            )
    ...
    def show_detailed_playlist_window(self) -> None:
        try:
            ...
            popup_content = self._create_popup_content_with_cache_support(playlist_info, cached_data)
            ...
            self.detailed_popup.bind(on_dismiss=lambda *_: self.cleanup_on_popup_close(playlist_id))
    ...
    def _extract_playlist_info(self) -> Dict[str, Any]:
        return {
            ...
            'owner_name': self.playlist_data.get('owner', {}).get('display_name', 'Unknown'),
            'owner_id': self.playlist_data.get('owner', {}).get('id', 'Unknown'),
            'owner_type': self.playlist_data.get('owner', {}).get('type', 'Unknown'),
            'owner_url': self.playlist_data.get('owner', {}).get('external_urls', {}).get('spotify', ''),
            'playlist_description': self.playlist_data.get('description', ''),
            'track_count': self.playlist_data.get('tracks', {}).get('total', 0),
            'playlist_id': self.playlist_data.get('id', ''),
            'spotify_url': self.playlist_data.get('external_urls', {}).get('spotify', ''),
            'followers_count': self.playlist_data.get('followers', {}).get('total') if self.playlist_data.get('followers') else None,
    ...
    def _create_popup_content_with_immediate_details(self, playlist_info):
        ...
        scroll_view = ScrollView(do_scroll_x=False, do_scroll_y=True, scroll_wheel_distance=dp(30), size_hint=(1, 1))
        scroll_view.add_widget(scroll_content)

        # Create info layout and image container
        info_layout = self._create_basic_info_section_only(playlist_info)
        image_container = playlist_info['image_container'] = self._create_image_section_with_immediate_tech(playlist_info)
        self.image_container = image_container  # Store as instance variable for cache refresh
    ...
    def _create_image_section_with_immediate_tech(self, playlist_info):
        image_container = BoxLayout(orientation='vertical', size_hint_x=None, width=dp(220))
        image_container.bind(minimum_height=image_container.setter('height'))

        image_url = self._get_playlist_image_url()
        if image_url:
            playlist_image = CachedAsyncImage(source=image_url, size_hint=(None, None), size=(dp(180), dp(180)), fit_mode='cover')
            ...
            playlist_image.bind(pos=lambda inst, value: setattr(playlist_image.canvas.before.children[-1], 'pos', value))
            ...
            try:
                ...
                if image_dimensions:
                    ...
                    if available_sizes and len(available_sizes) > 1:
                        image_dimensions += f" (Available: {', '.join(available_sizes)})"
        ...
        image_container.add_widget(Widget(size_hint_y=None, height=dp(15)))  # Reverted to original spacing
        tech_widgets = self.create_technical_details_widgets_for_left_side(
            playlist_info['owner_id'], playlist_info['playlist_id'], playlist_info['snapshot_id']
        )
        ...
        Clock.schedule_once(lambda _dt: image_container.do_layout(), 0.3)  # Extra refresh
        return image_container

    def _create_basic_info_section_only(self, playlist_info):
        info_layout = BoxLayout(orientation='vertical', spacing=dp(5), size_hint_y=None, padding=[0, 0, dp(5), 0])
        info_layout.bind(minimum_height=info_layout.setter('height'))

        self.master_content_container = BoxLayout(orientation='vertical', spacing=dp(5), size_hint_y=None)
        self.master_content_container.bind(minimum_height=self.master_content_container.setter('height'))

        def update_text_sizes(layout, size):
            try:
                actual_width = max(dp(280), size[0] - dp(40))
                for widget in layout.children:
                    if hasattr(widget, 'text_size') and widget.text_size and widget.text_size[0] != actual_width:
    ...
    def _add_basic_info_widgets(self, layout, playlist_info):
        ...
        layout.add_widget(self.create_info_label(owner_info, font_size=dp(16), height=dp(25)))
        ...
        description = re.sub(r'<[^>]+>', '', playlist_info['playlist_description'] or '').strip()
        if description:
            desc_header = Label(
                text='Description:',
                font_size=dp(18),
            ...
            desc_height = max(dp(20), min(dp(100), len(description) // 60 * dp(14) + dp(3)))
            desc_label = Label(
                text=description,
                font_size=dp(13),
                color=(0.8, 0.8, 0.8, 1),
        ...
        if playlist_info['is_collaborative']:
            playlist_type.append('Collaborative')
        layout.add_widget(self.create_info_label(f"Type: {', '.join(playlist_type)}", font_size=dp(16), height=dp(22)))
        layout.add_widget(Widget(size_hint_y=None, height=dp(4)))
    ...
    def create_technical_details_widgets_for_left_side(self, owner_id, playlist_id, snapshot_id):
        widgets: List[Widget] = []
        if owner_id != 'Unknown' or playlist_id or snapshot_id != 'Unknown':
            widgets.append(
                Label(
    ...
    def create_cache_statistics_widgets(self, playlist_id: str) -> List[Widget]:
        ...
        try:
            ...
            if cached_data:
                ...
                spotify_percentage = (spotify_cached / total_tracks * 100) if total_tracks > 0 else 0
                widgets.append(
                    Label(
                        text=f'Spotify: {spotify_cached}/{total_tracks} ({spotify_percentage:.0f}%)',
                        font_size=dp(11),
                        color=(0.2, 0.8, 0.2, 1) if spotify_percentage >= 80 else (0.8, 0.8, 0.2, 1) if spotify_percentage >= 50 else (0.8, 0.4, 0.4, 1),
                        size_hint_y=None,
                        height=dp(18),
                        text_size=(dp(215), None),
                        halign='left',
                ...
                reccobeats_percentage = (reccobeats_cached / total_tracks * 100) if total_tracks > 0 else 0
                widgets.append(
                    Label(
                        text=f'ReccoBeats: {reccobeats_cached}/{total_tracks} ({reccobeats_percentage:.0f}%)',
                        font_size=dp(11),
                        color=(0.2, 0.8, 0.2, 1) if reccobeats_percentage >= 80 else (0.8, 0.8, 0.2, 1) if reccobeats_percentage >= 50 else (0.8, 0.4, 0.4, 1),
                        size_hint_y=None,
                        height=dp(18),
                        text_size=(dp(215), None),
                        halign='left',
    ...
    def refresh_cache_statistics_display(self, playlist_id: str, force_refresh: bool = False) -> None:
        """Refresh the cache statistics display in the Details window."""
        try:
            logger.debug("CACHE REFRESH: Starting refresh for playlist %s (force: %s)", playlist_id, force_refresh)
            ...
            if not force_refresh:
                debounce_time = 0.5 if lightweight_mode else 1.0  # Faster refresh during lightweight mode
                
                if is_refreshing or (current_time - last_refresh < debounce_time):
                    logger.debug("CACHE REFRESH: Skipping - debounced (is_refreshing: %s, time_since: %.1fs, lightweight: %s, debounce: %.1fs)", 
                                is_refreshing, current_time - last_refresh, lightweight_mode, debounce_time)
                    return
                ...
                if token_info:
                    try:
                        ...
                        if sp:
                            ...
                            logger.debug("CACHE REFRESH: Retrieved user_id from Spotify API: %s", user_id)
                    except Exception as exc:
                        logger.debug("CACHE REFRESH: Error getting user_id from Spotify API: %s", exc)
            ...
            for widget in image_container.children:
                if isinstance(widget, Label):
                    if widget.text.startswith('Spotify:'):
                        spotify_percentage = int((spotify_cached_count / total_tracks * 100)) if total_tracks > 0 else 0
                        ...
                        if widget.text != new_text:
                            ...
                            widgets_updated += 1
                            logger.debug("Updated Spotify to %s (color: %s)", new_text, widget.color)
                    elif widget.text.startswith('ReccoBeats:'):
                        reccobeats_percentage = int((reccobeats_cached_count / total_tracks * 100)) if total_tracks > 0 else 0
                        ...
                        if widget.text != new_text:
                            ...
                            if reccobeats_percentage >= 80:
                                ...
                                widget.color = (0.8, 0.4, 0.4, 1)  # Red
                            widgets_updated += 1
                            logger.debug("Updated ReccoBeats to %s (color: %s)", new_text, widget.color)
            ...
            logger.debug("Cache refresh completed - updated %d widgets", widgets_updated)
            ...
            logger.debug("Successfully refreshed cache statistics display for playlist %s", playlist_id)
    ...
    def _lightweight_cache_update(self, image_container, playlist_id: str) -> None:
        ...
        try:
            logger.debug("LIGHTWEIGHT UPDATE: Starting lightweight update for playlist %s", playlist_id)
            ...
            from ..caching.track_cache import get_cached_spotify_track, get_cached_reccobeats_features
            ...
            logger.debug("LIGHTWEIGHT UPDATE: Real-time cache stats - Spotify: %d/%d, ReccoBeats: %d/%d (last_updated: %d)", 
                       spotify_cached_count, total_tracks, reccobeats_cached_count, total_tracks, last_updated)
            ...
            logger.debug("LIGHTWEIGHT UPDATE: Snapshot vs Real-time - Spotify: %d→%d, ReccoBeats: %d→%d", 
                       snapshot_spotify, spotify_cached_count, snapshot_reccobeats, reccobeats_cached_count)
            
            # Alert if there's a significant discrepancy
            if abs(spotify_cached_count - snapshot_spotify) > 5 or abs(reccobeats_cached_count - snapshot_reccobeats) > 5:
                logger.warning("LIGHTWEIGHT UPDATE: Cache snapshot is stale! Using real-time counts instead.")
            ...
            if getattr(self, '_lightweight_refresh_mode', False) and (spotify_cached_count > snapshot_spotify or reccobeats_cached_count > snapshot_reccobeats):
                    ...
                    if widget.text.startswith('Spotify:'):
                        ...
                        if widget.text != new_text:
                            ...
                            if spotify_percentage >= 80:
                                ...
                                widget.color = (0.8, 0.4, 0.4, 1)  # Red
                            widgets_updated += 1
                            logger.debug("LIGHTWEIGHT UPDATE: Updated Spotify to %s (color: %s)", new_text, widget.color)
                    elif widget.text.startswith('ReccoBeats:'):
                        reccobeats_percentage = int((reccobeats_cached_count / total_tracks * 100)) if total_tracks > 0 else 0
                        new_text = f'ReccoBeats: {reccobeats_cached_count}/{total_tracks} ({reccobeats_percentage}%)'
                        if widget.text != new_text:
                            ...
                            if reccobeats_percentage >= 80:
                                ...
                                widget.color = (0.8, 0.4, 0.4, 1)  # Red
                            widgets_updated += 1
                            logger.debug("LIGHTWEIGHT UPDATE: Updated ReccoBeats to %s (color: %s)", new_text, widget.color)
                    elif widget.text.startswith('Updated:'):
                        new_text = f'Updated: {last_updated_text}'
                        if widget.text != new_text:
                            widget.text = new_text
                            widgets_updated += 1
                            logger.debug("LIGHTWEIGHT UPDATE: Updated 'Updated' field to %s", new_text)
                    elif 'No cache data available' in widget.text:
                        no_cache_widget_found = True
                        logger.debug("LIGHTWEIGHT UPDATE: Found 'No cache data available' widget")
            
            # If we found "No cache data available" widget and now have cache data, replace it
            if no_cache_widget_found and (spotify_cached_count > 0 or reccobeats_cached_count > 0):
                logger.debug("LIGHTWEIGHT UPDATE: Replacing 'No cache data' widget with actual cache statistics")
                ...
                for widget in list(image_container.children):
                    if isinstance(widget, Label) and (
                        'No cache data available' in widget.text or 
                        (widget.text == 'Cache Status:' and no_cache_widget_found)  # Only remove header if it's the no-cache header
                    ):
                        widgets_to_remove.append(widget)
                
                for widget in widgets_to_remove:
                    image_container.remove_widget(widget)
                    logger.debug("LIGHTWEIGHT UPDATE: Removed widget: %s", widget.text[:30])
                
                # Create proper cache widgets
                new_cache_widgets = self.create_cache_statistics_widgets(playlist_id)
                logger.debug("LIGHTWEIGHT UPDATE: Adding %d new cache widgets", len(new_cache_widgets))
                
                # Add widgets in normal order (not reversed) to fix the order issue
                for widget in new_cache_widgets:
                    image_container.add_widget(widget)
                    widgets_updated += 1
                    logger.debug("LIGHTWEIGHT UPDATE: Added cache widget: %s", widget.text[:30])
                
                # Refresh layout to ensure proper positioning after cache updates
                Clock.schedule_once(lambda _dt: image_container.do_layout(), 0.1)
            
            logger.debug("LIGHTWEIGHT UPDATE: Completed - updated %d widgets", widgets_updated)
    ...
    def _load_tracks_immediately(self, playlist_info: Dict[str, Any], cached_data: Optional[Dict[str, Any]]) -> None:
            ...
            if token_info:
                ...
                if sp:
                    ...
                    if user_id:
                        ...
                        logger.info("Updated cache for playlist %s (Details window opened) - %d tracks, %d Spotify cached, %d ReccoBeats cached", 
                        ...
                        Clock.schedule_once(lambda dt: self.refresh_cache_statistics_display(playlist_id), 0.1)
                        Clock.schedule_once(lambda dt: self.refresh_cache_statistics_display(playlist_id), 0.8)
        except Exception as exc:
    ...
    def _load_tracks_with_progress(self, playlist_info: Dict[str, Any], cached_data: Optional[Dict[str, Any]], progress_window: ProgressWindow) -> None:
        ...
        def background_loading():
            try:
                ...
                if progress_window.is_cancelled:
                    logger.info("Track loading cancelled by user for playlist %s", playlist_name)
                    return
                ...
                def close_progress_and_start():
                    if not progress_window.is_cancelled:
                        ...
                        Clock.schedule_once(lambda dt: progress_window.dismiss(), 1.0)  # Auto-close after 1 second
    ...
    def _get_or_update_playlist_tracks_with_progress(self, sp, playlist_id: str, user_id: str, total_tracks: int, update_progress) -> Dict[str, Any]:
        """Get playlist tracks with progress updates."""
        try:
            # Get current tracks from Spotify API
            results = sp.playlist_tracks(playlist_id, limit=50)  # Smaller batch size for more frequent updates
            current_tracks = []
            processed = 0
            ...
            while results:
                ...
                for item in results['items']:
                    ...
                    if processed % 50 == 0 or processed >= total_tracks:
                        progress_percent = 10 + int((processed / total_tracks) * 75)  # 10% to 85%
                        update_progress(progress_percent, f"Loaded {processed:,}/{total_tracks:,} tracks...")
            ...
            return {
                ...
                'spotify_cached_count': cached_data.get('spotify_cached_count', 0) if cached_data else 0,
                'reccobeats_cached_count': cached_data.get('reccobeats_cached_count', 0) if cached_data else 0,
                'new_tracks': len(current_tracks) - (cached_data.get('total_tracks', 0) if cached_data else 0)
            }
    ...
    def fetch_analysis_with_cached_data_display(
        ...
        try:
            ...
            if not app.token_info or not app.token_info.get('access_token'):
                Clock.schedule_once(
                    lambda dt: self.update_loading_message(loading_label, 'Authentication error'),
                    0,
                )
                return
            ...
            if cached_data and cached_data.get('spotify'):
                ...
                if not tracks_data:
                    Clock.schedule_once(
                        lambda dt: self.update_loading_message(loading_label, 'No tracks found'), 0
                    )
                    return
                ...
                if hasattr(self, 'detailed_popup') and self.detailed_popup:
                    logger.debug("SPOTIFY COMPLETION: Forcing cache refresh for playlist %s", playlist_id)
                    setattr(self, '_last_cache_refresh', 0)  # Reset debouncing to force refresh
                    Clock.schedule_once(lambda dt: self.refresh_cache_statistics_display(playlist_id), 0.2)
                else:
                    logger.debug("SPOTIFY COMPLETION: No detailed popup found for playlist %s", playlist_id)
            ...
            Clock.schedule_once(
                lambda dt: self.update_loading_message(
                    playlist_info['loading_label'], f'Analysis error: {str(exc)[:30]}...'
                ),
                0,
            )
            ...
            while results:
                ...
                for item in results['items']:
                    ...
                    if track and track.get('type') == 'track':
                        ...
                        normalized_track = self._get_or_cache_spotify_track(track_payload)
                        if normalized_track:
                ...
                if results['next'] and not (analysis_task and analysis_task.is_cancelled()):
                    results = sp.next(results)
                else:
                    break
        except Exception as exc:
    ...
    def _get_or_cache_spotify_track(self, track: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        if cached_track:
            ...
            if playlist_id and user_id:
                ...
                persistent_cache.update_playlist_track_cache_status(playlist_id, user_id, track_id)
                ...
                if hasattr(self, 'detailed_popup') and self.detailed_popup:
                    ...
                    if not is_refreshing and not getattr(self, '_lightweight_refresh_mode', False):
                        # Only refresh if enough time has passed, or if this might be the first refresh
                        if current_time - last_refresh >= 2.0:  # Minimum 2 seconds between individual track refreshes
                ...
                logger.debug("Missing playlist_id or user_id - playlist_id: %s, user_id: %s", playlist_id, user_id)
        else:
    ...
    def _fetch_artist_data_cancellable(
        self, sp: spotipy.Spotify, artist_ids_list: List[str], analysis_task: AnalysisTask
    ...
    def _build_spotify_track_rows(self, tracks_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def group_genres(self, genre_counts: Dict[str, int]) -> Dict[str, int]:
        ...
        genre_keywords = {
            ...
            'Electronic/Dance': ['electronic', 'edm', 'house', 'techno', 'dance', 'dubstep'],
    ...
    def update_spotify_analysis(
        ...
        try:
            ...
            basic_widgets = [w for w in self.master_content_container.children if w != loading_label]
            ...
            if not skip_reccobeats:
                ...
                def schedule_periodic_refresh(dt):
                    logger.debug("PERIODIC REFRESH: Checking if should update (active=%s, popup=%s)", 
                               getattr(self, '_periodic_refresh_active', False), 
                               hasattr(self, 'detailed_popup') and self.detailed_popup)
                    if getattr(self, '_periodic_refresh_active', False) and hasattr(self, 'detailed_popup') and self.detailed_popup:
                        # Check if we're already refreshing to prevent conflicts
                        is_refreshing = getattr(self, '_is_refreshing', False)
                        if is_refreshing:
                            logger.debug("PERIODIC REFRESH: Skipping - already refreshing")
                        ...
                        lightweight_mode = getattr(self, '_lightweight_refresh_mode', False)
                        if lightweight_mode:
                            logger.debug("PERIODIC REFRESH: Forcing cache refresh during ReccoBeats processing")
                            Clock.schedule_once(lambda dt: self.refresh_cache_statistics_display(playlist_id, force_refresh=True), 0.1)
                        else:
                            # Only refresh if enough time has passed since last refresh (debouncing)
                            current_time = time.time()
                            last_refresh = getattr(self, '_last_cache_refresh', 0)
                            time_since_last = current_time - last_refresh
                            logger.debug("PERIODIC REFRESH: Time since last refresh=%.1fs (need 3.0s)", time_since_last)
                            if time_since_last >= 3.0:  # Maximum 1 refresh per 3 seconds
                                logger.debug("PERIODIC REFRESH: Scheduling cache refresh for playlist %s", playlist_id)
                                setattr(self, '_last_cache_refresh', current_time)
                                Clock.schedule_once(lambda dt: self.refresh_cache_statistics_display(playlist_id), 0.1)
                        # Schedule next refresh if still active and Details window is still open
                        Clock.schedule_once(schedule_periodic_refresh, 3.0)  # Check every 3 seconds
                    else:
                        logger.debug("PERIODIC REFRESH: Stopping - not active or no popup")
                
                # Start periodic refreshes after a longer delay to let ReccoBeats start
                logger.debug("PERIODIC REFRESH: Starting periodic refreshes in 5 seconds for playlist %s", playlist_id)
                ...
                setattr(self, '_lightweight_refresh_mode', True)  # Use lightweight mode during processing
                setattr(self, '_last_cache_refresh', 0)  # Initialize refresh timestamp
                
                threading.Thread(
                    target=self.fetch_reccobeats_with_cancellation,
                    args=(track_ids, reccobeats_loading_label, info_layout, owner_id, playlist_id, snapshot_id, analysis_task),
                    daemon=True,
                ).start()
    ...
    def _create_spotify_analysis_widgets(self, spotify_analysis: Dict[str, Any]) -> List[Widget]:
        widgets: List[Widget] = []
        if 'duration' in spotify_analysis and hasattr(self, 'stats_widget'):
            current_text = self.stats_widget.text
            self.stats_widget.text = f"{current_text} • Duration: {spotify_analysis['duration']}"
        else:
            widgets.append(
                self.create_info_label(
        ...
        if spotify_analysis.get('genre_analysis'):
            ...
            header = self.create_info_label('Genre Distribution:', font_size=dp(18), height=UIConstants.HEADER_HEIGHT)
            header.bold = True
            widgets.append(header)
            grouped = genre_data.get('grouped')
            if grouped:
                total = sum(grouped.values())
                for genre, count in sorted(grouped.items(), key=lambda x: x[1], reverse=True)[:5]:
        ...
        if spotify_analysis.get('artist_stats'):
            ...
            header = self.create_info_label('Artist Analysis:', font_size=dp(18), height=UIConstants.HEADER_HEIGHT)
            header.bold = True
            widgets.append(header)
            widgets.append(
                self.create_info_label(
            ...
            if artist_data.get('most_frequent'):
                ...
                widgets.append(
                    self.create_info_label(
                        'Top Artists: ' + ', '.join([f'{artist} ({count})' for artist, count in top_artists]),
                        font_size=dp(13),
                    )
                )
    ...
    def fetch_reccobeats_with_cancellation(
        ...
        try:
            ...
            if cached_data is not None and cached_data.get('reccobeats'):
                Clock.schedule_once(
                    lambda dt: self.update_reccobeats_analysis_from_cache(
                        cached_data['reccobeats'], info_layout, owner_id, playlist_id, snapshot_id
                    ),
                    0,
                )
                setattr(self, '_periodic_refresh_active', False)
            ...
            Clock.schedule_once(
                lambda dt: self._debug_spacer_action("add_after_spacer", "After API message update"),
                0.1,
            )

            if analysis_task and analysis_task.is_cancelled():
                fetched_count = len([row for row in (cached_data or {}).get('reccobeats', {}).get('track_rows', [])])
                logger.info(
            ...
            if audio_features_map:
                ...
                if user_id:
                    ...
                    for track_id in audio_features_map.keys():
                        persistent_cache.update_playlist_track_cache_status(playlist_id, user_id, track_id)
                        tracks_updated += 1
                    logger.debug("Updated cache status for %d/%d ReccoBeats tracks", tracks_updated, len(audio_features_map))
                    
                    # Debug: Check cache state immediately after update
                    updated_cache_data = persistent_cache.get_cached_playlist_tracks(playlist_id, user_id)
                    if updated_cache_data:
                        logger.debug("CACHE CHECK AFTER UPDATE: Spotify: %d/%d, ReccoBeats: %d/%d", 
                ...
                if hasattr(self, 'detailed_popup') and self.detailed_popup:
                    ...
                    Clock.schedule_once(lambda dt: self.refresh_cache_statistics_display(playlist_id, force_refresh=True), 0.2)
                else:
                    # logger.debug("Details window not open, skipping cache refresh")
                    pass
            ...
            reccobeats_analysis = self.analyze_reccobeats_features(audio_features_map, track_ids)
            if analysis_task and analysis_task.is_cancelled():
                setattr(self, '_periodic_refresh_active', False)
                setattr(self, '_lightweight_refresh_mode', False)
                return
            ...
            setattr(self, '_lightweight_refresh_mode', False)  # Disable lightweight mode for final refresh
            if hasattr(self, 'detailed_popup') and self.detailed_popup:
                setattr(self, '_last_cache_refresh', 0)  # Reset debouncing to force refresh
                Clock.schedule_once(lambda dt: self.refresh_cache_statistics_display(playlist_id), 0.2)
            ...
            setattr(self, '_periodic_refresh_active', False)
            setattr(self, '_lightweight_refresh_mode', False)  # Disable lightweight mode
    ...
    def _add_cached_reccobeats_widgets(
        ...
        try:
            dummy_label = Label(text='ReccoBeats: Analysis complete (from cache)', color=(0.2, 0.8, 0.2, 1))
    ...
    def analyze_reccobeats_features(
        ...
        try:
            ...
            results: Dict[str, Any] = {
                ...
                'tempo_analysis': {},
                'musical_keys': {},
                'coverage': len(audio_features) / len(track_ids) * 100 if track_ids else 0,
            ...
            for feature in feature_names:
                values = [f.get(feature) for f in audio_features if f and f.get(feature) is not None]
                if values:
            ...
            valence_values = [f.get('valence') for f in audio_features if f and f.get('valence') is not None]
            energy_values = [f.get('energy') for f in audio_features if f and f.get('energy') is not None]
            ...
            results['musical_keys'] = self.analyze_musical_keys_reccobeats(audio_features)
    ...
    def analyze_musical_keys_reccobeats(self, audio_features: List[Dict[str, Any]]) -> Dict[str, Any]:
        try:
            ...
            return {
                ...
                'coverage_percentage': int((total_with_keys / len(audio_features)) * 100)
                if audio_features
                else 0,
    ...
    def update_reccobeats_analysis(
        ...
        try:
            ...
            existing_widgets = [w for w in self.master_content_container.children if w != loading_label]
            self.master_content_container.clear_widgets()
            ...
            if reccobeats_data.get('no_features'):
                ...
                if mood:
                    header = self.create_info_label('Playlist Mood (ReccoBeats):', font_size=dp(18), height=dp(24))
                    header.bold = True
                    widgets.append(header)
                ...
                if tempo or (musical_keys and not musical_keys.get('unavailable')):
                    header = self.create_info_label('Musical Characteristics (ReccoBeats):', font_size=dp(18), height=UIConstants.HEADER_HEIGHT)
                    header.bold = True
                    widgets.append(header)
                    if tempo:
                        widgets.append(
                    ...
                    if musical_keys and musical_keys.get('top_combinations'):
                        ...
                        combo_text = 'Most Common Keys: ' + ', '.join([f"{name} ({count})" for name, count in combos])
                        widgets.append(self.create_info_label(combo_text, height=UIConstants.STANDARD_SPACING))
                ...
                if features:
                    header = self.create_info_label('Audio Characteristics (ReccoBeats):', font_size=dp(18), height=UIConstants.HEADER_HEIGHT)
                    header.bold = True
                    widgets.append(header)
            ...
            if hasattr(self, 'detailed_popup') and self.detailed_popup:
                ...
                if main_container and len(main_container.children) > 1:
                    ...
                    if getattr(scroll_view, 'children', None):
                        ...
                        if hasattr(scroll_view, '_update_effect_bounds'):
                            scroll_view._update_effect_bounds()
                        Clock.schedule_once(lambda _: setattr(scroll_view, 'scroll_y', 1.0), 0.1)
    ...
    def show_tooltip_positioned_on_card(self, mouse_pos) -> None:
        try:
            ...
            try:
                ...
                tooltip_x = max(dp(5), min(tooltip_x, Window.width - tooltip_width - dp(5)))
                if tooltip_y + tooltip_height > Window.height - dp(10):
                    tooltip_y = mouse_y - tooltip_height - dp(10)
                tooltip_y = max(dp(5), min(tooltip_y, Window.height - tooltip_height - dp(5)))
    ...
    def init_checkbox_graphics(self, dt) -> None:
        try:
            ...
            with self.checkbox.canvas.before:
                Color(1, 1, 1, 0.98)
                self.checkbox_bg = Rectangle(size=(dp(16), dp(16)), pos=(self.checkbox.x, self.checkbox.y))
        except Exception as exc:
    ...
    def on_checkbox_change(self, checkbox, value) -> None:
        try:
            ...
            with self.checkbox.canvas.before:
                Color(1, 1, 1, 0.98)
                self.checkbox_bg = Rectangle(size=(dp(18), dp(18)), pos=(self.checkbox.x, self.checkbox.y))
        except Exception as exc:
