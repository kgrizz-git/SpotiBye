    ...
    def recalculate_columns(self, *args):
        try:
            if self.width > 0 and abs(self.width - self._last_width) > dp(10):
                ...
                max_cols = max(1, int(available_width / (self.min_card_width + self.spacing[0])))
                
                # Calculate actual width per card
                card_width = (available_width - ((max_cols - 1) * self.spacing[0])) / max_cols
                
                # If cards would be too wide, reduce columns
                if card_width > self.max_card_width:
                    max_cols = max(1, int(available_width / (self.max_card_width + self.spacing[0])))
                    card_width = min(self.max_card_width, 
                                   (available_width - ((max_cols - 1) * self.spacing[0])) / max_cols)
                
                if max_cols != self._cached_cols or abs(self._last_card_width - card_width) > 1:
                    self.cols = max_cols
    ...
    def add_widget(self, widget, *args, **kwargs):
        ...
        if hasattr(self, '_cached_cols'):
            ...
            if available_width > 0 and self._cached_cols > 0:
                card_width = (available_width - ((self._cached_cols - 1) * self.spacing[0])) / self._cached_cols
