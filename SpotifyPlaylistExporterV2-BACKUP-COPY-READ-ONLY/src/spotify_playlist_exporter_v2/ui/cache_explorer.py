...
class CacheExplorerPopup(Popup):
    ...
    def _create_header(self) -> BoxLayout:
        ...
        # Search input
        search_label = Label(
            text="Search:", size_hint_x=None, width=dp(60), font_size=dp(14)
        )
    ...
    def _create_tracks_column(self) -> BoxLayout:
        ...
        header = BoxLayout(orientation="horizontal", size_hint_y=None, height=dp(30))
        header_label = Label(
            text="Tracks", font_size=dp(16), bold=True, color=(0.3, 0.6, 0.8, 1)
        )
        header.add_widget(header_label)
    ...
    def _create_details_column(self) -> BoxLayout:
        ...
        header = BoxLayout(orientation="horizontal", size_hint_y=None, height=dp(30))
        header_label = Label(
            text="Track Details", font_size=dp(16), bold=True, color=(0.8, 0.6, 0.3, 1)
        )
        header.add_widget(header_label)
    ...
    def populate_tracks_column(self, playlist: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not cached_data:
                basic_playlist_data = cache.get_cached_playlist_data(
                    playlist_id, user_id
                )
                if basic_playlist_data and "tracks" in basic_playlist_data:
                    ...
                    for track_item in basic_playlist_data["tracks"].get("items", []):
                        ...
                        if track:
                            basic_track = {
                                ...
                                "spotify_cached": cache.get_cached_track_data(
                                    track.get("id"), "spotify"
                                )
                                is not None,
                                "reccobeats_cached": cache.get_cached_track_reccobeats(
                                    track.get("id")
                                )
                                is not None,
                            }
                    ...
                    if tracks:
                        logger.info(
                            "Loaded %d tracks from basic playlist cache for %s",
                            len(tracks),
                            playlist_id,
                        )
                    else:
                        no_data = Label(
                            text="No tracks found in playlist cache", font_size=dp(14)
                        )
                        self.tracks_content.add_widget(no_data)
                        return
                else:
                    no_data = Label(
                        text="No cached tracks found for this playlist\n(Try opening playlist details first)",
                        font_size=dp(14),
                    )
                    self.tracks_content.add_widget(no_data)
                    return
    ...
    def select_track(self, track: Dict[str, Any]) -> None:
        ...
        reccobeats_cached = track.get("reccobeats_cached", False)
        logger.debug(
            "CACHE EXPLORER: Selected track %s - ReccoBeats cached: %s",
            spotify_id,
            reccobeats_cached,
        )
    ...
    def populate_details_column(self, track: Dict[str, Any]) -> None:
        ...
        details = [
            ...
            (
                "Artists",
                ", ".join([str(artist) for artist in artists if artist is not None])
                if artists
                else "Unknown",
            ),
            (
    ...
    def _create_detail_item(self, label: str, value: str) -> BoxLayout:
        ...
        value_widget = Label(
            text=value, font_size=dp(12), halign="left", text_size=(None, None)
        )
    ...
    def populate_features_column(self, track: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(
                "CACHE EXPLORER: Getting ReccoBeats data for track: %s", spotify_id
            )
            ...
            logger.debug(
                "CACHE EXPLORER: ReccoBeats data returned: %s", type(track_data)
            )
            if track_data:
                logger.debug(
                    "CACHE EXPLORER: ReccoBeats data keys: %s", list(track_data.keys())
                )
            ...
            for feature_name, value, format_str in feature_items:
                feature_widget = self._create_feature_item(
                    feature_name, value, format_str
                )
    ...
    def _create_feature_item(self, name: str, value: Any, format_str: str) -> BoxLayout:
        ...
        widget = BoxLayout(
            orientation="horizontal", size_hint_y=None, height=dp(25), spacing=dp(5)
        )
