...
class MainScreen(Screen):
    ...
    def _create_export_section(self) -> BoxLayout:
        ...
        self.status_label = Label(
            text="Ready to export", font_size=dp(14), size_hint_y=None, height=dp(30)
        )
        ...
        button_container = BoxLayout(
            orientation="horizontal", size_hint_x=None, width=dp(200), spacing=dp(5)
        )
    ...
    def _perform_sort(self) -> None:
        ...
        try:
            ...
            states = {
                w.playlist_data.get("id", ""): w.checkbox.active
            ...
            for playlist in sorted_playlists:
                try:
                    ...
                    widget.checkbox.bind(
                        active=lambda *_: self.update_selection_counter()
                    )
    ...
    def _export_playlists_worker(self, playlist_widgets):
        ...
        try:
            ...
            self._update_export_status(
                f"Export complete. Files saved to: {os.path.abspath(SAVE_DIR)}"
            )
    ...
    def start_export(self, *_args) -> None:
        try:
            selected = [
                w.playlist_data for w in self.playlist_widgets if w.checkbox.active
            ]
            if not selected:
                Popup(
                    title="No Selection",
    ...
    def _apply_table_style(self, worksheet, header_row: int) -> None:
        ...
        base_name = (
            re.sub(r"[^A-Za-z0-9_]", "", f"tbl_{worksheet.title}") or "PlaylistTable"
        )
