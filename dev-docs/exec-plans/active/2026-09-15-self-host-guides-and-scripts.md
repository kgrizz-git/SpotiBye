...
- [ ] Spotify app registration detail: exact Redirect URI per mode (local loopback vs deployed Worker URL), where to find Client ID/Secret, the read-only scope list above so users know what they are approving, the app-owner Premium requirement (Feb 2026 rules: app stops working if the owner's Premium lapses), and the User Management allowlist flow for each user.
