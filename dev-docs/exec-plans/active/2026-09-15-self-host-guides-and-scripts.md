...
- [x] Implement (Python, stdlib only, lives in `scripts/`):
  ...
  - [x] Refuse to overwrite an existing `.dev.vars` or generated config without `--force`; assert `.dev.vars` is gitignored (verify, don't assume) and ensure generated files stay gitignored.
