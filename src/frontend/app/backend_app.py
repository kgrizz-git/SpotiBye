...
from ..screens.backend_main_screen_adapter import (
...
class BackendSpotifyExporterApp(MDApp):
    ...
    def _initialize_backend(self, backend_url: str) -> None:
        ...
        try:
            ...
            original_logger.info(
                f"Backend initialized with URL: {self.selected_backend_url}"
            )
    ...
    def build(self):  # type: ignore[override]
        try:
            ...
            diagnose_macos_issues()
            set_window_basics(
                "Spotify Playlist Exporter - Powered by ReccoBeats & Cloudflare"
            )
    ...
    def switch_to_main(self) -> None:
        ...
        if self.screen_manager:
            ...
            if (
                hasattr(self.main_screen, "initialize_with_backend")
                and self.backend_adapter
            ):
                self.main_screen.initialize_with_backend(self.backend_adapter)
            Clock.schedule_once(
                lambda _dt: self._check_pending_export_after_login(), 0.2
            )
    ...
    def get_backend_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_stats = (
                self.cache_manager.get_cache_stats() if self.cache_manager else {}
            )
    ...
    def _check_pending_export_after_login(self) -> None:
        ...
        is_completed = str(status.get("status") or "") == "completed" or not bool(
    ...
    def _show_pending_export_popup(
        self,
        ...
        title = (
            "Completed Export Available" if is_completed else "Resume Pending Export"
        )
        ...
        button_row = BoxLayout(
            orientation="horizontal", spacing=dp(12), size_hint_y=None, height=dp(42)
        )
        ...
        def _resume(*_args: Any) -> None:
            _dismiss()
            if (
                hasattr(self.main_screen, "begin_backend_export")
                and self.backend_adapter
            ):
    ...
    def refresh_backend_connection(self) -> None:
        ...
        try:
            ...
            if self.backend_adapter and hasattr(
