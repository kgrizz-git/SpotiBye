...
class TestPerformance:
    ...
    def test_large_playlist_loading(self) -> bool:
        ...
        try:
            ...
            if len(tracks) <= 1000:
                self.framework.end_test(False, f"Playlist not large enough: {len(tracks)} tracks")
                return False
            ...
            self.framework.end_test(True, f"Loaded {len(tracks)} tracks in {load_time:.2f}s")
            return True
    ...
    def test_analysis_performance(self) -> bool:
        ...
        try:
            ...
            if analysis_time > 60.0:  # 60 seconds
                self.framework.end_test(False, f"Analysis too slow: {analysis_time:.2f}s")
                return False
        ...
        try:
            ...
            def load_playlist(playlist_id):
                try:
                    ...
                    tracks = self.backend_client.get_playlist_tracks(playlist_id)
                    load_time = time.time() - start_time
                    results.put({'success': True, 'tracks': len(tracks), 'time': load_time})
                except Exception as e:
            ...
            if success_count < len(playlists[:3]):
                self.framework.end_test(False, f"Only {success_count}/{len(playlists[:3])} requests succeeded")
                return False
            
            self.framework.end_test(True, f"All {success_count} concurrent requests succeeded")
            return True
    ...
    def test_memory_usage(self) -> bool:
        ...
        try:
            ...
            if memory_increase > 500:  # 500 MB increase
                self.framework.end_test(False, f"Memory usage too high: {memory_increase:.1f}MB increase")
                return False
            
            self.framework.end_test(True, f"Memory usage acceptable: {memory_increase:.1f}MB increase")
            return True
