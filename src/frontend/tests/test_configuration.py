...
class TestConfiguration:
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            self.original_env = {
                ...
                "SPOTIBYE_PRODUCTION_BACKEND_URL": os.environ.get(
    ...
    def test_development_environment_config(self) -> bool:
        ...
        try:
            ...
            self.framework.end_test(
                True, "Development environment configuration working"
            )
            return True
    ...
    def test_production_environment_config(self) -> bool:
        ...
        try:
            ...
            self.framework.end_test(
                True, "Production environment configuration working"
            )
            return True
    ...
    def test_backend_client_configuration(self) -> bool:
        ...
        try:
            ...
            if default_client.base_url != CURRENT_BACKEND_URL:
                self.framework.end_test(
                    False,
                    f"Default client URL should be {CURRENT_BACKEND_URL}, got {default_client.base_url}",
                )
                return False
            ...
            self.framework.end_test(
                False, f"Backend client configuration test failed: {e}"
            )
            return False
        ...
        try:
            ...
            with patch.object(BackendCacheManager, "__init__", return_value=None):
                ...
                if str(cache_manager.cache_dir) != custom_cache_dir:
                    self.framework.end_test(
                        False,
                        f"Cache manager should use custom dir {custom_cache_dir}, got {cache_manager.cache_dir}",
                    )
                    return False
            ...
            self.framework.end_test(
                False, f"Cache directory configuration test failed: {e}"
            )
            return False
        ...
        try:
            ...
            if hasattr(UIConstants, "ERROR_MESSAGE_COLOR"):
                if not isinstance(UIConstants.ERROR_MESSAGE_COLOR, str):
                    self.framework.end_test(
                        False, "ERROR_MESSAGE_COLOR should be string"
                    )
                    return False
            ...
            self.framework.end_test(
                False, f"Feature flags configuration test failed: {e}"
            )
            return False
    ...
    def test_oauth_configuration(self) -> bool:
        ...
        try:
            ...
            if custom_auth.callback_port != 9999:
                self.framework.end_test(
                    False,
                    f"Authenticator should accept custom port 9999, got {custom_auth.callback_port}",
                )
                return False
    ...
    def test_environment_switching(self) -> bool:
        ...
        try:
            ...
            with patch.dict("sys.modules"):
                ...
                if backend_config.CURRENT_BACKEND_URL != "http://dev-backend:8787":
                    self.framework.end_test(
                        False, "Should switch back to development backend"
                    )
                    return False
