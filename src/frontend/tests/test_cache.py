...
class TestCachePerformance:
    ...
    def test_cache_hit_rate(self) -> bool:
        ...
        try:
            ...
            if len(cached_playlists) != len(test_playlists):
                self.framework.end_test(False, f"Cache size mismatch: expected {len(test_playlists)}, got {len(cached_playlists)}")
                return False
            
            # Test performance - cache retrieval should be fast
            if retrieve_time > 0.1:  # 100ms threshold
                self.framework.end_test(False, f"Cache retrieval too slow: {retrieve_time:.3f}s")
                return False
            ...
            self.framework.end_test(True, f"Cache hit rate: 100%, cache time: {cache_time:.3f}s, retrieve time: {retrieve_time:.3f}s")
            return True
    ...
    def test_cache_ttl_performance(self) -> bool:
        ...
        try:
            ...
            is_valid = self.cache_manager.is_playlists_cache_valid()
            if not is_valid:
                self.framework.end_test(False, "Cache should be valid immediately after caching")
                return False
            ...
            self.framework.end_test(True, f"TTL performance test passed, stats: {stats}")
            return True
    ...
    def test_cache_memory_usage(self) -> bool:
        ...
        try:
            ...
            stats = self.cache_manager.get_cache_stats()
            if not stats:
                self.framework.end_test(False, "No cache statistics after large dataset")
                return False
            
            # Verify all items are cached
            if stats['playlists_count'] != 10:
                self.framework.end_test(False, f"Expected 10 cached items, got {stats['playlists_count']}")
                return False
            ...
            if len(cached_data) != 10:
                self.framework.end_test(False, f"Retrieved {len(cached_data)} items, expected 10")
                return False
            
            # Performance should still be reasonable
            if retrieve_time > 0.2:  # 200ms threshold for large dataset
                self.framework.end_test(False, f"Large dataset retrieval too slow: {retrieve_time:.3f}s")
                return False
            ...
            self.framework.end_test(True, f"Memory usage test passed, cache_time: {cache_time:.3f}s, retrieve_time: {retrieve_time:.3f}s, cached_items: 10")
            return True
    ...
    def test_concurrent_cache_access(self) -> bool:
        ...
        try:
            ...
            # Test data
            test_data = [{'id': f'concurrent_{i}', 'name': f'Concurrent Test {i}'} for i in range(50)]
            ...
            while not results.empty():
                ...
                if result['success']:
                    ...
                    logger.error(f"Worker {result['worker_id']} failed: {result['error']}")
            
            if successful_workers < 5:
                self.framework.end_test(False, f"Only {successful_workers}/5 workers succeeded")
                return False
            ...
            if avg_retrieve_time > 0.5:  # 500ms threshold
                self.framework.end_test(False, f"Concurrent access too slow: {avg_retrieve_time:.3f}s")
                return False
            
            self.framework.end_test(True, f"Concurrent access test passed, workers: {successful_workers}/5, avg_cache_time: {avg_cache_time:.3f}s, avg_retrieve_time: {avg_retrieve_time:.3f}s")
            return True
