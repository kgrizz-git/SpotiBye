...
class TestUIResponsiveness:
    ...
    def test_loading_indicators_during_requests(self) -> bool:
        ...
        try:
            ...
            with patch('time.sleep', return_value=0.01):  # Speed up test
                ...
                self.mock_ui_update('loading_completed', {'operation': 'get_playlists', 'result_count': len(playlists)})
            
            # Verify loading indicators were shown
            loading_events = [e for e in self.ui_events if e['event_type'] in ['loading_started', 'loading_completed']]
            
            if len(loading_events) < 2:
                self.framework.end_test(False, f"Expected at least 2 loading events, got {len(loading_events)}")
                return False
            
            # Verify proper sequence: start -> complete
            start_event = next((e for e in loading_events if e['event_type'] == 'loading_started'), None)
            complete_event = next((e for e in loading_events if e['event_type'] == 'loading_completed'), None)
            
            if not start_event or not complete_event:
                self.framework.end_test(False, "Missing loading start or complete events")
                return False
            
            if start_event['timestamp'] > complete_event['timestamp']:
                self.framework.end_test(False, "Loading start event after complete event")
                return False
            
            self.framework.end_test(True, f"Loading indicators working properly, events: {len(loading_events)}")
            return True
    ...
    def test_ui_freezing_prevention(self) -> bool:
        ...
        try:
            ...
            if not ui_responsive:
                self.framework.end_test(False, "UI became unresponsive during operation")
                return False
            ...
            if len(ui_events) < 5:  # Should have at least 5 UI updates
                self.framework.end_test(False, f"Insufficient UI updates: {len(ui_events)}")
                return False
            
            self.framework.end_test(True, f"UI remained responsive, updates: {len(ui_events)}, operation_time: {operation_time:.3f}s")
            return True
    ...
    def test_progress_indicators(self) -> bool:
        ...
        try:
            ...
            progress_events = [e for e in self.ui_events if e['event_type'] == 'progress_update']
            
            if len(progress_events) != 5:
                self.framework.end_test(False, f"Expected 5 progress events, got {len(progress_events)}")
                return False
            ...
            for i, event in enumerate(progress_events):
                ...
                if progress.get('current_step', 0) != expected_step:
                    self.framework.end_test(False, f"Current step incorrect at event {i}: expected {expected_step}, got {progress.get('current_step', 0)}")
                    return False
                
                if abs(progress.get('progress', 0) - expected_progress) > 0.01:
                    self.framework.end_test(False, f"Progress value incorrect at event {i}: expected {expected_progress}, got {progress.get('progress', 0)}")
                    return False
            
            self.framework.end_test(True, f"Progress indicators working, events: {len(progress_events)}")
            return True
    ...
    def test_error_handling_ui_feedback(self) -> bool:
        ...
        try:
            ...
            def handle_network_error(error: Exception):
                self.mock_ui_update('error_occurred', {
            ...
            error_events = [e for e in self.ui_events if e['event_type'] == 'error_occurred']
            
            if len(error_events) != 1:
                self.framework.end_test(False, f"Expected 1 error event, got {len(error_events)}")
                return False
            ...
            if not error_data['retry_available']:
                self.framework.end_test(False, "Retry should be available for network errors")
                return False
            
            self.framework.end_test(True, f"Error handling UI feedback working, error_type: {error_data['error_type']}")
            return True
            
        except Exception as e:
            self.framework.end_test(False, f"Error handling UI feedback test failed: {e}")
            return False
        ...
        try:
            ...
            def network_operation(operation_id: int):
                ...
                try:
                    ...
                    self.mock_ui_update('operation_completed', {
                        'id': operation_id,
                        'result_count': len(playlists)
                    })
                except Exception as e:
                    self.mock_ui_update('operation_failed', {'id': operation_id, 'error': str(e)})
            ...
            if ui_update_count < 10:  # Should have at least 10 UI updates
                self.framework.end_test(False, f"Insufficient UI updates during concurrent operations: {ui_update_count}")
                return False
            
            # Verify operation events
            operation_events = [e for e in self.ui_events if 'operation' in e['event_type']]
            
            if len(operation_events) < 6:  # Should have at least 6 operation events (3 started + 3 completed)
                self.framework.end_test(False, f"Insufficient operation events: {len(operation_events)}")
                return False
            
            self.framework.end_test(True, f"Concurrent operations working, ui_updates: {ui_update_count}, operation_events: {len(operation_events)}")
            return True
