...
class MockBackendHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == "/health":
            ...
            "/tracks" in parsed_path.path or "/items" in parsed_path.path
        ):
            ...
            parsed_path.path.startswith("/analysis/playlist/")
            and "/status" in parsed_path.path
        ):
            playlist_id = parsed_path.path.split("/")[3]
            self._handle_get_analysis_status(playlist_id)
        elif (
            parsed_path.path.startswith("/analysis/playlist/")
            and "/results" in parsed_path.path
        ):
            playlist_id = parsed_path.path.split("/")[3]
    ...
    def _handle_get_playlist_details(self, playlist_id: str):
        ...
        playlist = self.test_data.get("playlist_details", {}).get(
            playlist_id,
            {
                "id": playlist_id,
    ...
    def _handle_start_analysis(self, playlist_id: str):
        ...
        response = {"job_id": job_id, "status": "pending", "playlist_id": playlist_id}
        self._send_json_response(200, response)
    ...
    def _handle_get_analysis_status(self, playlist_id: str):
        ...
        current_status = self.test_data.get("analysis_status", {}).get(
            playlist_id, "running"
        )
    ...
    def _handle_get_analysis_results(self, playlist_id: str):
        ...
        results = self.test_data.get("analysis_results", {}).get(
            playlist_id,
            {
                "playlist_id": playlist_id,
...
DEFAULT_TEST_DATA = {
    ...
    "analysis_status": {"small_playlist": "completed", "large_playlist": "running"},
    "analysis_results": {
        "small_playlist": {
