...
class PersistentCache:
    ...
    # Playlist cache
    # ------------------------------------------------------------------
    def cache_playlist_data(self, playlist_id: str, playlist_data: Dict[str, Any], user_id: Optional[str] = None) -> None:
        try:
    ...
    def get_cached_playlist_data(self, playlist_id: str, user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        try:
            cache_key = self._get_cache_key(playlist_id, user_id)
    ...
    def cache_image(self, image_url: str, image_data: bytes) -> Optional[str]:
        try:
            ...
            self._save_metadata()
            logger.debug("Cached optimized image: %s (%d bytes)", image_url, len(image_data))
    ...
    def _optimize_image(self, image_data: bytes, image_url: str) -> Optional[bytes]:
        ...
        try:
            ...
            if len(optimized_data) < len(image_data) * 0.9:  # At least 10% reduction
                logger.debug("Optimized image: %d -> %d bytes (%.1f%% reduction)", 
                           len(image_data), len(optimized_data),
    ...
    # Analysis cache
    # ------------------------------------------------------------------
    def cache_analysis_data(self, playlist_id: str, analysis_data: Dict[str, Any], analysis_type: str = 'combined') -> None:
        try:
    ...
    def get_cached_analysis_data(self, playlist_id: str, analysis_type: str = 'combined') -> Optional[Dict[str, Any]]:
        try:
            cache_key = f"{playlist_id}_{analysis_type}"
    ...
    def _persist_track_cache(self, spotify_id: str, cache_payload: Dict[str, Any]) -> None:
    ...
    def cache_track_data(
        ...
        try:
            ...
            logger.warning("Error caching %s data for track %s: %s", source, spotify_id, exc)

    def get_cached_track_data(self, spotify_id: str, source: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            logger.warning("Error retrieving cached %s data for %s: %s", source, spotify_id, exc)
            return None
    ...
    def _remove_track_source(
        ...
        try:
            ...
            cache_payload = cache_payload or self._load_track_cache(spotify_id, cache_file)
            ...
            logger.warning("Error removing cached %s data for %s: %s", source, spotify_id, exc)

    def cache_track_reccobeats(self, spotify_id: str, reccobeats_data: Dict[str, Any]) -> None:
    ...
    def cache_playlist_tracks(self, playlist_id: str, user_id: str, tracks: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            for track in tracks:
                ...
                spotify_cached = self.get_cached_track_data(track_id, 'spotify') is not None
                reccobeats_cached = self.get_cached_track_reccobeats(track_id) is not None
            ...
            cache_data = {
                ...
                'spotify_cached_count': sum(1 for t in processed_tracks if t['spotify_cached']),
                'reccobeats_cached_count': sum(1 for t in processed_tracks if t['reccobeats_cached']),
            ...
            logger.info("Cached %d tracks for playlist %s (%d Spotify cached, %d ReccoBeats cached)", 
                       len(processed_tracks), playlist_id, 
                       cache_data['spotify_cached_count'], cache_data['reccobeats_cached_count'])
                       
        except Exception as exc:
            logger.error("Error caching playlist tracks: %s", exc)

    def get_cached_playlist_tracks(self, playlist_id: str, user_id: str) -> Optional[Dict[str, Any]]:
    ...
    def update_playlist_track_cache_status(self, playlist_id: str, user_id: str, track_id: str) -> None:
        ...
        try:
            ...
            for track in cached_data['tracks']:
                if track['id'] == track_id:
                    track['spotify_cached'] = self.get_cached_track_data(track_id, 'spotify') is not None
                    track['reccobeats_cached'] = self.get_cached_track_reccobeats(track_id) is not None
                    break
            
            # Update counts and save
            cached_data['spotify_cached_count'] = sum(1 for t in cached_data['tracks'] if t['spotify_cached'])
            cached_data['reccobeats_cached_count'] = sum(1 for t in cached_data['tracks'] if t['reccobeats_cached'])
    ...
    def cleanup_old_cache(self) -> None:
        try:
            ...
            for cache_key in list(tracks.keys()):
                cache_info = tracks[cache_key]
                cache_file = Path(cache_info['file_path'])
                spotify_id = cache_info.get('spotify_id') or cache_key.replace('track_', '')
                cache_payload = self._load_track_cache(spotify_id, cache_file)
                sources = cache_payload.get('sources', {})
                for source_name in list(sources.keys()):
                    fetched_at = sources[source_name].get('fetched_at', cache_info.get('cached_at', 0))
                    if not fetched_at or not self._is_cache_valid(fetched_at):
                        self._remove_track_source(spotify_id, source_name, cache_payload, cache_file)
                        cleaned_count += 1
                        cache_payload = self._load_track_cache(spotify_id, cache_file)
                        sources = cache_payload.get('sources', {})
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        try:
            ...
            for directory in [self.data_cache_dir, self.image_cache_dir, self.analysis_cache_dir]:
    ...
    def clear_all_cache(self) -> None:
        try:
            ...
            for directory in [self.cache_dir, self.data_cache_dir, self.image_cache_dir, self.analysis_cache_dir]:
                directory.mkdir(parents=True, exist_ok=True)
    ...
    def get_detailed_cache_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for cache_key, cache_info in self.metadata['playlists'].items():
                try:
                    playlist_data = self.get_cached_playlist_data(
                        cache_info.get('playlist_id', ''), 
                        cache_info.get('user_id')
                    )
                    name = playlist_data.get('name', f'Playlist {cache_info.get("playlist_id", "Unknown")}')
                ...
                detailed_info['playlists'].append({
                    ...
                    'size_mb': round(self._get_file_size_mb(cache_info.get('file_path', '')), 2)
                })
            ...
            for cache_key, cache_info in tracks_sample:
                try:
                    spotify_id = cache_info.get('spotify_id', cache_key.replace('track_', ''))
                    ...
                    if track_file.exists():
                        try:
                            ...
                            spotify_data = track_data.get('sources', {}).get('spotify', {})
                            ...
                            logger.debug("Error reading track cache %s: %s", cache_key, exc)
