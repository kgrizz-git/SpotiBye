...
class InsufficientDiskSpaceError(ExportError):
    ...
    def __init__(self, free_space: int, required_space: int):
        ...
        super().__init__(
            f"Insufficient disk space: {required_space // (1024*1024)}MB required, {free_space // (1024*1024)}MB available"
        )
...
class MainScreen(Screen):
    ...
    def export_selected(self, instance):
        ...
        selected_playlists = [
            ...
            if w.playlist_data.get("id") in self.selected_playlist_ids
        ]
        ...
        threading.Thread(
            ...
            args=(selected_playlists, selected_format),
            daemon=True,
        ).start()
    ...
    def _export_playlists_worker(self, playlist_widgets, format_type):
        ...
        try:
            ...
            total_tracks = sum(
                len(w.playlist.get("tracks", [])) for w in playlist_widgets
            )
            ...
            for i, widget in enumerate(playlist_widgets):
                ...
                self._update_export_status(
                    f'Exporting {i+1}/{len(playlist_widgets)}: {playlist["name"]}...'
                )

                try:
                    track_data = self._prepare_playlist_track_rows(
                        sp, playlist, job_state=None, include_reccobeats=True
                    )
                    rows = track_data["combined_rows"]
                    if not rows:
                        logger.warning(
                            "No rows generated for playlist %s", playlist.get("name")
                        )
                        continue
                    ...
                    internal_cols = [
                        col for col in df.columns if str(col).startswith("_")
                    ]
                    if internal_cols:
                    ...
                    if format_type == "csv":
                        try:
                            ...
                            raise ExportFormatError(
                                f"CSV export failed for '{playlist['name']}': {str(e)}"
                            )
                        ...
                        try:
                            ...
                            raise ExportFormatError(
                                f"JSON export failed for '{playlist['name']}': {str(e)}"
                            )
                    else:  # xlsx (default)
                        try:
                            ...
                            if df.empty:
                                raise ExportFormatError(
                                    "Cannot export empty DataFrame to Excel"
                                )
                            ...
                            raise ExportFormatError(
                                f"Excel export failed for '{playlist['name']}': {str(e)}"
                            )

                    self._update_export_status(
                        f'Exported: {playlist["name"]} ({len(df)} tracks)'
                    )

                except ExportFormatError as e:
                    # Format-specific error - log but continue with other playlists
                    self._log_error(
                        str(e), {"playlist": playlist["name"], "format": format_type}
                    )
                    self._update_export_status(
                        f"Format error exporting {playlist['name']}: {str(e)}"
                    )
                    continue
                except Exception as e:
                    # Unexpected error for specific playlist - log but continue
                    self._log_error(
                        f"Unexpected error exporting playlist {playlist['name']}: {str(e)}"
                    )
                    self._update_export_status(
                        f"Error exporting {playlist['name']}: {str(e)}"
                    )
                    continue

            self._update_export_status(
                f"Export completed: {len(playlist_widgets)} playlists exported as {format_type.upper()}"
            )
            ...
            self._show_error_dialog(
                "Permission Denied",
                "Cannot write to selected location. Please check file permissions.",
            )
            ...
            self._show_error_dialog(
                "Export Failed", "An unexpected error occurred during export."
            )
    ...
    def _format_excel_file(self, file_path, playlist_name):
        ...
        try:
            ...
            if not os.path.exists(file_path):
                raise ExportFormatError(
                    f"Excel file not found for formatting: {file_path}"
                )
    ...
    def load_playlists_with_cache(self, *_args) -> None:
        ...
        threading.Thread(
            target=self.load_playlists_worker_with_cache, daemon=True
        ).start()
    ...
    def load_playlists_worker_with_cache(self) -> None:
        try:
            ...
            if not token_info:
                Clock.schedule_once(
                    lambda _: setattr(self.status_label, "text", "Please login again"),
                    0,
                )
                return
            ...
            while results:
                for playlist in results["items"]:
                    playlist_id = playlist.get("id")
                    cached = persistent_cache.get_cached_playlist_data(
                        playlist_id, user_id
                    )
                    if cached:
                        playlists.append(cached)
                    else:
                        persistent_cache.cache_playlist_data(
            ...
            if exc.http_status == 401:
                ...
                Clock.schedule_once(
                    lambda _: setattr(self.status_label, "text", error_msg), 0
                )
        except Exception as exc:
    ...
    def display_playlists_with_cache(self) -> None:
        ...
        try:
            ...
            self.selected_playlist_ids = {
                pid for pid in self.selected_playlist_ids if pid in current_ids
            }
    ...
    def update_status_with_cache_info(self) -> None:
        ...
        try:
            ...
            cache_info = (
                f" • Cache: {cache_size:.1f}MB ({cache_files} files)"
                if cache_size
                else ""
            )
    ...
    def _check_cache_status_and_proceed(self, playlists) -> None:
        ...
        try:
            ...
            for playlist in playlists:
                ...
                try:
                    ...
                    if cached_data:
                        ...
                        reccobeats_cached_count = cached_data.get(
                            "reccobeats_cached_count", 0
                        )

                        # Tracks missing Reccobeats data are those not cached for ReccoBeats
                        playlist_tracks_missing_reccobeats = (
                            len(tracks) - reccobeats_cached_count
                        )
                    ...
                    logger.warning(
                        "Error checking cache for playlist %s: %s", playlist_id, exc
                    )
            ...
            self._show_feature_selection_popup(
                playlists, tracks_missing_reccobeats, total_tracks
            )
            ...
            self._show_feature_selection_popup(
                playlists,
                0,
                sum(p.get("tracks", {}).get("total", 0) for p in playlists),
            )

    def _show_feature_selection_popup(
        self, playlists, tracks_missing_reccobeats, total_tracks
    ) -> None:
        ...
        question_text = (
            "Include Reccobeats features (tempo, musical key, other features)?"
        )
        ...
        no_btn.bind(
            on_press=lambda *_: self._handle_feature_selection_response(
                popup, playlists, False
            )
        )
        yes_btn.bind(
            on_press=lambda *_: self._handle_feature_selection_response(
                popup, playlists, True
            )
        )
        popup.open()

    def _handle_feature_selection_response(
        self, popup, playlists, include_reccobeats
    ) -> None:
    ...
    def _proceed_with_export_after_cache_check(
        self, playlists, include_reccobeats=True
    ) -> None:
        ...
        if os.path.exists(output_path):
            self._show_overwrite_confirmation(
                playlists, output_path, filename, include_reccobeats
            )
        else:
            self.begin_export(playlists, output_path, include_reccobeats)

    def _show_overwrite_confirmation(
        self, playlists, output_path, filename, include_reccobeats=True
    ) -> None:
        popup_content = BoxLayout(
            orientation="vertical", spacing=dp(10), padding=dp(20)
        )
        ...
        cancel_btn.bind(on_press=lambda *_: popup.dismiss())
        overwrite_btn.bind(
            on_press=lambda *_: self._handle_overwrite_confirmed(
                popup, playlists, output_path, include_reccobeats
            )
        )
        popup.open()

    def _handle_overwrite_confirmed(
        self, popup, playlists, output_path, include_reccobeats=True
    ) -> None:
        popup.dismiss()
        self.begin_export(playlists, output_path, include_reccobeats)
    ...
    def begin_export(self, playlists, output_path, include_reccobeats=True) -> None:
        try:
            ...
            threading.Thread(
                ...
                args=(playlists, output_path, include_reccobeats),
                daemon=True,
            ).start()
        except Exception as exc:
    ...
    def export_worker(self, playlists, output_path, include_reccobeats=True) -> None:
        ...
        try:
            ...
            if not sp:
                ...
                return
            Clock.schedule_once(
                lambda _: setattr(self.status_label, "text", "Creating Excel file..."),
                0,
            )
            ...
            with pd.ExcelWriter(
                ...
                for index, playlist in enumerate(playlists):
                    ...
                    progress = int(
                        (index / max(total, 1)) * 40
                    )  # Adjusted progress range
                    Clock.schedule_once(
                        lambda _, p=progress: setattr(self.progress_bar, "value", p), 0
                    )
                    playlist_name = playlist.get("name", "Unknown Playlist")
                    Clock.schedule_once(
                        lambda _, name=playlist_name: setattr(
                            self.status_label, "text", f"Processing: {name[:30]}..."
                        ),
                        0,
                    )

                    track_data = self._prepare_playlist_track_rows(
                        sp, playlist, current_export_job, include_reccobeats
                    )
                    ...
                    duration_str = self._format_duration_from_ms(
                        track_data["total_duration_ms"]
                    )
                    ...
                    sheet_name = self._sanitize_sheet_name(
                        f"{playlist_name} - {playlist.get('owner', {}).get('display_name', 'Unknown')}"
                    )
                    ...
                    if cover_url:
                        try:
                            temp_path = self.download_and_cache_image(
                                cover_url, playlist_meta["name"]
                            )
                            if temp_path:
                    ...
                    try:
                        ...
                        title_cell = worksheet.cell(
                            row=1, column=1, value=playlist_meta["name"]
                        )
                ...
                if worksheet is not None:
                    for row_idx, row in enumerate(
                        worksheet.iter_rows(min_row=11, min_col=5, max_col=5), start=11
                    ):
                        for cell in row:
                            if (
                                cell.value
                                and isinstance(cell.value, str)
                                and cell.value.startswith("https://")
                            ):
            ...
            if sheets_created == 0:
                logger.warning(
                    "No playlists with tracks found, creating a default sheet"
                )
                df = pd.DataFrame(
                    [["No tracks found in any playlists", "", "", "", ""]],
            ...
            if sheets_created > 0:
                Clock.schedule_once(
                    lambda _: setattr(
                        self.status_label, "text", "Adding table formatting..."
                    ),
                    0,
                )
                Clock.schedule_once(
                    lambda _: setattr(self.progress_bar, "value", 95), 0
                )
            ...
            Clock.schedule_once(
                lambda _: setattr(
                    self.status_label, "text", f"Export complete! Saved: {filename}"
                ),
                0,
            )
            Clock.schedule_once(
                lambda _: self._show_completion_popup(
            ...
            Clock.schedule_once(
                lambda _, msg=error_message: setattr(
                    self.status_label, "text", f"Export failed: {msg[:50]}..."
                ),
                0,
            )
            Clock.schedule_once(lambda _: self._refresh_filename_after_export(), 0)
            ...
            for temp_file in temp_files:
                try:
                    ...
                    logger.warning(
                        "Error removing temporary file %s: %s", temp_file, exc
                    )
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if features and any(features):  # Check if we got any features back
                        ...
                        logger.warning(
                            f"Empty audio features response on attempt {attempt + 1}"
                        )
                except Exception as e:
                    logger.warning(
                        f"Error fetching audio features (attempt {attempt + 1}): {e}"
                    )
                    if attempt == max_retries - 1:  # Last attempt failed
                        logger.error(
                            "Max retries reached for audio features, continuing without them"
                        )
                        break
        ...
        try:
            ...
            while True:
                ...
                for item in results["items"]:
                    ...
                    key = (
                        key_map[audio_features.get("key", 0)]
                        if "key" in audio_features
                        else "N/A"
                    )
                    mode = (
                        mode_map[audio_features.get("mode", 1)]
                    ...
                    tracks.append(
                        {
                            ...
                            "Spotify URL": track.get("external_urls", {}).get(
                                "spotify", ""
                            ),
                            "Tempo": round(audio_features.get("tempo", 0), 2)
                            ...
                            "Danceability": round(
                                audio_features.get("danceability", 0), 3
                            )
                            ...
                            "Instrumentalness": round(
                                audio_features.get("instrumentalness", 0), 3
                            )
                            if "instrumentalness" in audio_features
                            else "N/A",
                            "Liveness": round(audio_features.get("liveness", 0), 3)
                            if "liveness" in audio_features
                            else "N/A",
                            "Speechiness": round(
                                audio_features.get("speechiness", 0), 3
                            )
                            if "speechiness" in audio_features
                            else "N/A",
                            "Loudness": round(audio_features.get("loudness", 0), 1)
                            if "loudness" in audio_features
                            else "N/A",
                            "Time Signature": audio_features.get(
                                "time_signature", "N/A"
                            )
    ...
    def _prepare_playlist_track_rows(
        ...
        try:
            ...
            for track in current_tracks:
                ...
                normalized = get_cached_spotify_track(
                    spotify_id
                ) or cache_spotify_track(track)
                if not normalized:
                    artists = [
        ...
        if missing_reccobeats and include_reccobeats:
            try:
                feature_map = reccobeats_api.get_multiple_track_audio_features_safe(
                    missing_reccobeats
                )
                ...
                logger.warning(
                    "Error fetching ReccoBeats audio features during export: %s", exc
                )
    ...
    def _build_export_row_from_spotify(
        self, normalized_track: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _apply_reccobeats_to_row(
        self, row: Dict[str, Any], features: Dict[str, Any]
    ) -> None:
    ...
    def _apply_table_style(self, worksheet, header_row: int) -> None:
        ...
        try:
            ...
            base_name = (
                re.sub(r"[^A-Za-z0-9_]", "", f"tbl_{worksheet.title}")
                or "PlaylistTable"
            )
            ...
            logger.warning(
                f"Error applying table style to worksheet '{worksheet.title}': {e}"
            )
            # Continue even if table styling fails

    def _format_playlist_sheet(
        self, worksheet, auto_resize: bool = False
    ) -> Optional[int]:
        column_widths = {
            "A": 30,
            "B": 40,
            ...
            for row in range(header_row + 1, worksheet.max_row + 1):
                ...
                if isinstance(url, str) and url.startswith("http"):
                    try:
                        ...
                        logger.warning(
                            f"Error converting URL to hyperlink at row {row}: {e}"
                        )
                        continue

            logger.debug(
                f"Converted {converted_count} URLs to hyperlinks in worksheet '{worksheet.title}'"
            )

        except Exception as exc:
            logger.warning(
                f"Error converting URLs to hyperlinks in worksheet '{worksheet.title}': {exc}"
            )
    ...
    def _get_playlist_metadata(self, playlist):
        ...
        owner_display = owner_info.get("display_name") or owner_info.get(
            "id", "Unknown"
        )
        followers = (
            playlist.get("followers", {}).get("total")
    ...
    def download_and_cache_image(self, image_url, playlist_name):
        ...
        try:
            ...
            temp_fd, temp_path = tempfile.mkstemp(
                prefix="playlist_cover_", suffix=".img"
            )
            ...
            logger.warning(
                "Error downloading playlist image for %s: %s", playlist_name, exc
            )
            return None
    ...
    def _show_completion_popup(self, filename, output_path):
        ...
        success_label.bind(
            width=lambda *x: setattr(
                success_label, "text_size", (success_label.width, None)
            )
        )
        ...
        if not is_mobile:
            ...
            question.bind(
                width=lambda *x: setattr(question, "text_size", (question.width, None))
            )
            ...
            open_btn.bind(
                on_press=lambda *_: self._handle_open_file_request(popup, output_path)
            )
        else:
            mobile_text = Label(
    ...
    def _handle_open_file_request(self, popup, file_path):
        ...
        if not self._open_file_with_system_default(file_path):
            Popup(
                ...
                content=Label(
                    text="Could not open the file with the system default application.\n\nPlease navigate to the file location and open it manually.",
    ...
    def purge_cache(self, *_args) -> None:
        ...
        confirm_btn = Button(
            text="Clear Cache", size_hint_x=0.5, background_color=[0.8, 0.3, 0.3, 1]
        )
