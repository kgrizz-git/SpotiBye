...
class ReccoBeatsAPI:
    ...
    def __init__(self) -> None:
        ...
        self.base_url = settings.RECCOBEATS_BASE_URL
    ...
    def get_reccobeats_id_from_spotify_id(
        ...
        try:
            ...
            response = self.session.get(
                url, params=params, timeout=settings.RECCOBEATS_TIMEOUT
            )
            ...
            if response.status_code == 404:
                logger.debug(
                    "Track %s not found in ReccoBeats database", spotify_track_id
                )
            elif response.status_code == 429:
                logger.warning(
                    "ReccoBeats API rate limit hit for track lookup %s",
                    spotify_track_id,
                )
            else:
                logger.warning(
            ...
            logger.warning(
                "ReccoBeats track lookup API timeout for %s", spotify_track_id
            )
            return ""
        except requests.exceptions.RequestException as exc:
            logger.warning(
                "ReccoBeats track lookup API request error for %s: %s",
                spotify_track_id,
                exc,
            )
            return ""
            ...
            logger.error(
                "Unexpected error getting ReccoBeats ID for %s: %s",
                spotify_track_id,
                exc,
            )
            return ""
    ...
    def _extract_reccobeats_id(self, spotify_track_id: str, data: Any) -> str:
        ...
        if tracks_data:
            ...
            if reccobeats_id:
                logger.debug(
                    "Found ReccoBeats ID %s for Spotify ID %s",
                    reccobeats_id,
                    spotify_track_id,
                )
    ...
    def get_audio_features_by_reccobeats_id(
        ...
        try:
            ...
            response = self.session.get(url, timeout=settings.RECCOBEATS_TIMEOUT)
            ...
            if response.status_code == 200:
                ...
                if any(feature in data for feature in expected_features):
                    return data
                logger.warning(
                    "ReccoBeats response missing expected audio features for %s",
                    reccobeats_id,
                )
                return {}
            ...
            if response.status_code == 404:
                logger.debug(
                    "Audio features not found for ReccoBeats ID %s", reccobeats_id
                )
            elif response.status_code == 429:
                logger.warning(
                    "ReccoBeats API rate limit hit for audio features %s", reccobeats_id
                )
            else:
                logger.warning(
            ...
            logger.warning(
                "ReccoBeats audio features API timeout for %s", reccobeats_id
            )
            return {}
        except requests.exceptions.RequestException as exc:
            logger.warning(
                "ReccoBeats audio features API request error for %s: %s",
                reccobeats_id,
                exc,
            )
            return {}
            ...
            logger.error(
                "Unexpected error getting audio features for %s: %s", reccobeats_id, exc
            )
            return {}
    ...
    def get_multiple_reccobeats_ids(
        ...
        try:
            total_batches = (
                len(spotify_track_ids) + max_batch_size - 1
            ) // max_batch_size
            logger.info(
                "Starting ReccoBeats ID lookup for %s tracks (%s batches)",
            ...
            for i in range(0, len(spotify_track_ids), max_batch_size):
                ...
                if i % 50 == 0 or batch_num % 10 == 1:
                    logger.info(
                        "Processing ReccoBeats ID lookup batch %s/%s (tracks %s-%s)",
                        batch_num,
                        total_batches,
                        i + 1,
                        min(i + max_batch_size, len(spotify_track_ids)),
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "Error processing batch %s: %s", i // max_batch_size + 1, exc
                    )
                    continue
    ...
    def _process_batch_response(
        ...
        if not tracks_data:
            logger.debug(
                "No tracks data found in batch response for batch size %s", len(batch)
            )
            return
    ...
    def _get_multiple_track_audio_features_impl(
        ...
        try:
            ...
            logger.info(
                "Step 1: Getting ReccoBeats IDs for %s tracks", len(spotify_track_ids)
            )
            ...
            if missing_ids:
                logger.info(
                    "Looking up ReccoBeats IDs for %s uncached tracks", len(missing_ids)
                )
                fresh_mapping = self.get_multiple_reccobeats_ids(
                    missing_ids, analysis_task=analysis_task
                )
            ...
            logger.info(
                "Step 2: Getting audio features for %s mapped tracks", len(id_mapping)
            )
            ...
            if use_cache:
                for spotify_id in spotify_ids:
                    cached_data = persistent_cache.get_cached_track_reccobeats(
                        spotify_id
                    )
                    if cached_data:
                        cached_count += 1
                ...
                logger.info(
                    "Found %s cached ReccoBeats features, fetching %s uncached tracks",
                    cached_count,
                    len(uncached_tracks),
                )

            def fetch_audio_features(
                spotify_id: str, reccobeats_id: str
                ...
                if use_cache:
                    ...
                    if cached_data:
                        logger.debug(
                            "Using cached ReccoBeats data for track: %s", spotify_id
                        )
                ...
                for attempt in range(3):
                    try:
                        ...
                        features = self.get_audio_features_by_reccobeats_id(
                            reccobeats_id, analysis_task
                        )
                        ...
                        if features:
                            ...
                            if use_cache:
                                persistent_cache.cache_track_reccobeats(
                                    spotify_id, features
                                )
                        ...
                        if attempt == 2:
                            logger.warning(
                                "Failed to get audio features for %s after 3 attempts: %s",
                                spotify_id,
                                exc,
                            )
                            return spotify_id, {}
            ...
            if spotify_ids:
                logger.info(
                    "Starting audio features fetch for %s uncached tracks (%s batches)",
                    len(spotify_ids),
                    total_batches,
                )
            else:
                logger.info(
                    "All %s tracks already cached, no fetching needed", cached_count
                )
                return results
            ...
            for i in range(0, len(spotify_ids), batch_size):
                if analysis_task and analysis_task.is_cancelled():
                    logger.info(
                        "ReccoBeats audio features cancelled during batch processing"
                    )
                    return results
                ...
                if i % 50 == 0 or batch_num % 10 == 1:
                    logger.info(
                        "Processing audio features batch %s/%s (tracks %s-%s, %s features found so far)",
                        batch_num,
                        total_batches,
                        i + 1,
                        min(i + batch_size, len(spotify_ids)),
                        len(results),
                    )
                ...
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    future_to_track = {
                        executor.submit(
                            fetch_audio_features, spotify_id, id_mapping[spotify_id]
                        ): spotify_id
                    ...
                    for future in concurrent.futures.as_completed(future_to_track):
                        if analysis_task and analysis_task.is_cancelled():
                            logger.info(
                                "ReccoBeats audio features cancelled during future processing"
                            )
                        ...
                        try:
                            spotify_id, features = future.result(
                                timeout=settings.RECCOBEATS_TIMEOUT * 3
                            )
                            if features:
                            ...
                            logger.warning(
                                "Error processing audio features for %s: %s",
                                spotify_id,
                                exc,
                            )
                ...
                if processed_count % 50 == 0 or processed_count == len(spotify_ids):
                    logger.info(
                        "Audio features progress: %s/%s tracks processed (%s features found)",
                        processed_count,
                        len(spotify_ids),
                        len(results),
                    )
            ...
            success_rate = (
                len(results) / len(spotify_track_ids) * 100 if spotify_track_ids else 0
            )
            logger.info(
