...
async def start_export(
    ...
    token_info: Optional[Dict[str, Any]] = Depends(lambda: None)
) -> Dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start export: {str(exc)}")
    ...
    try:
        ...
        try:
            ...
            return StreamingResponse(
                ...
                headers={"Content-Disposition": f"attachment; filename=playlist_export_{export_id}.xlsx"}
            )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to download export: {str(exc)}")
    ...
    try:
        ...
        for track in tracks:
            row = {
                ...
                'Album Artist': track['album']['artists'][0]['name'] if track['album']['artists'] else '',
        ...
        if include_audio_features:
            ...
            for col in feature_columns:
                df[col] = df['Track ID'].map(lambda x: features_data.get(x, {}).get(col, ''))
        ...
        if include_reccobeats_analysis:
            ...
            if cached_analysis and cached_analysis.get('reccobeats'):
                ...
                recco_columns = [
                    'recco_energy', 'recco_valence', 'recco_danceability',
                    'recco_acousticness', 'recco_instrumentalness'
                ]
                
                for col in recco_columns:
                    df[col] = df['Track ID'].map(lambda x: reccobeats_data.get(x, {}).get(col, ''))
        ...
        if format.lower() == 'csv':
            ...
            return StreamingResponse(
                ...
                media_type="text/csv",
                headers={"Content-Disposition": f"attachment; filename={playlist_name}_export.csv"}
            )
            ...
            return StreamingResponse(
                ...
                headers={"Content-Disposition": f"attachment; filename={playlist_name}_export.json"}
            )
            ...
            return StreamingResponse(
                output,
                media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                headers={"Content-Disposition": f"attachment; filename={playlist_name}_export.xlsx"}
            )
        
    except Exception as exc:
        logger.error(f"Error in immediate export: {exc}")
        raise HTTPException(status_code=500, detail=f"Failed to export playlist: {str(exc)}")
...
async def export_playlist_background(
    ...
    try:
        ...
        logger.info(f"Background export {export_id} completed for playlist: {playlist_id}")
