...
from ..caching.analysis import AnalysisTask, cache_playlist_analysis, get_cached_playlist_analysis
...
async def start_playlist_analysis(
    ...
    try:
        ...
        if playlist_id in AnalysisTask.active_analysis_tasks:
            raise HTTPException(
                status_code=409, 
                detail="Analysis already in progress for this playlist"
            )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start analysis: {str(exc)}")
...
async def get_analysis_status(playlist_id: str) -> AnalysisStatusResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get analysis status: {str(exc)}")
...
async def get_analysis_results(playlist_id: str) -> Dict[str, Any]:
    ...
    try:
        cached_data = get_cached_playlist_analysis(playlist_id)
        if not cached_data:
            raise HTTPException(status_code=404, detail="No analysis found for this playlist")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get analysis results: {str(exc)}")
...
async def cancel_analysis(playlist_id: str) -> Dict[str, str]:
    ...
    try:
        if playlist_id in AnalysisTask.active_analysis_tasks:
            ...
            raise HTTPException(status_code=404, detail="No active analysis found for this playlist")
            
    except HTTPException:
        raise
    except Exception as exc:
        logger.error(f"Error cancelling analysis: {exc}")
        raise HTTPException(status_code=500, detail=f"Failed to cancel analysis: {str(exc)}")
...
async def get_track_audio_features(
    ...
    try:
        ...
        if not features or not features[0]:
            raise HTTPException(status_code=404, detail="Audio features not found for track")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get audio features: {str(exc)}")


async def analyze_playlist_background(
    playlist_id: str, 
    include_reccobeats: bool, 
