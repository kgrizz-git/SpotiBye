...
class Settings(BaseSettings):
    ...
    def _validate_settings(self) -> None:
        ...
        if (
            not self.SPOTIPY_CLIENT_ID
            or self.SPOTIPY_CLIENT_ID == "YOUR_CLIENT_ID_HERE"
        ):
            raise ValueError("SPOTIPY_CLIENT_ID must be set")

        if (
            not self.SPOTIPY_CLIENT_SECRET
            or self.SPOTIPY_CLIENT_SECRET == "YOUR_CLIENT_SECRET_HERE"
        ):
        ...
        if (
            ...
            if self.DEBUG:
                ...
                logging.warning(
                    "Using default test secret key - not suitable for production"
                )
            else:
