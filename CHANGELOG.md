...
### Added
- Interactive self-host setup script (`python3 scripts/setup-selfhost.py`) for the local path: checks prerequisites, opens the Spotify Developer Dashboard, validates Spotify credentials via a live token check before writing anything, and generates `src/backend/.dev.vars`. Supports `--check`, `--dry-run`, and `--force`; secret values are never printed. (Cloudflare provisioning/upload/config helpers exist and are unit-tested but not yet wired into the interactive flow.)
- The backend chooser now starts unconfigured when no saved or explicitly configured endpoint exists. Localhost remains an intentional selectable preset, Cloudflare Dev/Prod presets appear only when their URL variables are configured, and cancelling an unconfigured chooser no longer connects to localhost.
- Export enrichment columns (Tempo, Key, Danceability, Energy, Valence, Acousticness, Instrumentalness, Liveness, Speechiness, Loudness) are populated from the global per-track ReccoBeats cache on export — including first export without prior analysis via on-demand miss-fill. **Time Signature** remains permanent `N/A` (ReccoBeats does not provide it). Single-export KV keys now include format and enrichment variant (`export:{playlistId}:{userId}:{format}:{enriched|plain}`) so enriched exports do not collide with legacy unenriched cache entries.
- `force_enrichment` on `POST /analysis/playlist/{id}` bypasses idempotent completed/queued short-circuit, clears user analysis status/results, and propagates through the analysis queue to per-track ReccoBeats cache force-resolve. Successful force enrichment clears the single-export KV prefix (`export:{playlistId}:{userId}*`); in-flight resumable export job/batch keys are not cleared.
...
- Playlist analysis popups now show live progress with a progress bar and status text while backend analysis runs, including bounded activity feedback when Cloudflare KV status reads are stale. Dismissing the popup stops local progress polling; the backend job keeps running server-side.
- Large-playlist analysis now fans out into distributed per-batch queue messages so no Worker invocation exceeds the subrequest budget (previously large playlists lost all ReccoBeats enrichment to `Too many subrequests`); small playlists keep the single-message path unchanged.
...
- Playlist analysis section is more readable: larger section headers and body text with breathing room between sections.
- Partial-data notes in playlist analysis are smaller and per-chunk coverage notes are tallied into one line (e.g. "1 of 27 tracks" instead of three "of 9" lines).
...
- Manual analysis refresh is now **Refresh track info** (uses `force_enrichment` instead of DELETE + POST) and bypasses the per-session automatic miss-fill guard.
- Playlist track cache TTL increased from 2h to 24h with composition fingerprint metadata (`snapshot_id`, `track_id_hash`, `unique_track_count`). **Refresh playlist tracks** bypasses both local and backend KV caches via `force_refresh=true`.
- Analysis opening now uses offline enrichment completeness (endpoint resolved counts vs unique track IDs) instead of raw coverage ratios; incomplete playlists auto-trigger one targeted backend miss-fill per session, and composition changes (when online) invalidate stale analysis and refresh tracks. documented the previously-undocumented `GET /analysis/playlist/{id}/status` and `GET /analysis/playlist/{id}/results` endpoints, removed schema fields for never-implemented features (`duration_minutes`, `average_bpm`, `bpm_distribution`, `energy_score`, `recommendations`/`SimilarPlaylist`, `include_recommendations`), and added the real response shape (`overview`, `artists`, `genre_distribution`, `audio_features` with `key_mode_distribution`, `insights`, `reccobeats_metadata`, `errors`, `schema_version`).
...
- Reopening a playlist no longer re-runs the full analysis when the cached results are fresh but enrichment-partial (e.g. tracks ReccoBeats has no data for): the backend now serves the cached results, and explicit refresh paths (miss-fill, composition change, Refresh button) enqueue a `refresh` job that reuses the warm per-track cache instead of burning a full job every open.
...
- Auto-login no longer leaves the main-screen header as `Logged in as: None`. Startup session restore now hydrates `app.username` from `/auth/me` (with cached-username fallback), and the header treats a missing name as `Unknown User` instead of stringifying Python `None`.
- Playlist analysis refresh controls and enrichment status UX (B3): incomplete enrichment shows neutral progress copy instead of error banners when miss-fill is running; verified ReccoBeats omissions show complete-with-omissions status.
- Playlist analysis now has a manual **Refresh track info** action (formerly Retry enrichment) that force-refreshes per-track ReccoBeats data; coverage-only warnings no longer trigger full automatic reanalysis loops.
...
- Cleared the weekly Security Scan failure on main (27 Jul 2026) caused by stale advisory findings: bumped `pillow>=12.3.0` (PYSEC-2026-2253 through PYSEC-2026-3496, CVSS up to 8.7) and consolidated the npm `brace-expansion` overrides from stale per-major pins (1.1.16 / 2.1.2 / 5.0.7) to a single `5.0.8` (GHSA-mh99-v99m-4gvg), which also collapses four duplicate lockfile entries into one. Added an explicit `postcss` override (`^8.5.18`, GHSA-r28c-9q8g-f849). Bumped the OSV-Scanner GitHub Action (sub-action `c5185470…` → `9a498708…`, v2.3.8) and the pre-push hook (`v2.3.5` → `v2.3.8`) to keep the CLI engine in parity across CI and local checks. Backend test suite (92 files / 848 tests), build, and lint remain green.
