...
### Added
- Interactive self-host setup script (`python3 scripts/setup-selfhost.py`): checks prerequisites, opens the Spotify/Cloudflare portal pages, validates Spotify credentials via a live token check before writing anything, and generates `src/backend/.dev.vars`. Supports `--check`, `--dry-run`, and `--force`; secret values are never printed.
