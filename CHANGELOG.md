...
### Changed
- Playlist track cache TTL increased from 2h to 24h with composition fingerprint metadata (`snapshot_id`, `track_id_hash`, `unique_track_count`). **Refresh playlist tracks** (B3) bypasses both local and backend KV caches via `force_refresh=true`.
